...
public class World {
	...
	private class Move {
	    ...
	    public Move(int row, int col, boolean opened, boolean flagged) {
	        this.row = row;
	        this.col = col;
	        this.opened = opened;
	    ...
	    public void undo() {
	        if (opened) {
	            arrayBoolean[row][col] = false;
	            arrayButton[row][col].setNumber(-1);
	        }
	        if (flagged) {
	            arrayCamCo[row][col] = false;
	            arrayButton[row][col].setNumber(-1);
	        }
	    }
	}
	...
	public void camCo(int i, int j) {
		if (!arrayBoolean[i][j]) {
			if (arrayCamCo[i][j]) {
			 ...
			 undoStack.push(new Move(i, j, false, arrayCamCo[i][j]));
		}
	...
	public boolean open(int i, int j) {
		...
		if (!isComplete && !isEnd) {
			...
			if (arrayMin[i][j] == -1) {
				...
				undoStack.push(new Move(i, j, arrayBoolean[i][j], false));
				return false;
			} else {
